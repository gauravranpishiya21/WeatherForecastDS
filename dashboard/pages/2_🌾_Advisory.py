...
st.set_page_config(page_title="Advisory", page_icon="🌾", layout="wide", initial_sidebar_state="collapsed")
from dashboard.components.theme import inject_weather_theme, navigate_if_needed
inject_weather_theme(hide_sidebar=True)
navigate_if_needed("Agro-Advisory")
...
def _advisory_live(lat: float, lon: float) -> dict | None:
    try:
        resp = httpx.post(f"{API_BASE}/api/advisory",
...
def _advisory_demo(snap: dict, village_id: str) -> dict | None:
    ...
    if stage is None:
        ...
        if info and info.growth_stages:
            ...
            st.error(f"Crop '{crop}' not found.")
            return None

    daily = [{"date": d["date"], "temperature": d["temp_max"], "rainfall": d["rainfall_mm"],
              "humidity": d["humidity"], "wind_speed": d["wind_kmh"]} for d in v["daily"]]

    crop_adv = engine.generate_advisory(crop, stage, daily[0])
    assessor = WeatherRiskAssessor()
    risks = assessor.assess_risks(daily)
    llm = LLMAdvisoryGenerator(api_key=None)
    ...
    for d in daily[:7]:
        ...
        prio = "High" if da.risk_level in ("HIGH", "CRITICAL") else "Medium" if da.risk_level == "MODERATE" else "Low"
        summary = "; ".join(a.advice[:60] for a in da.action_items[:2])
        weekly.append({"date": d["date"], "action": summary or "Continue routine practices", "priority": prio})

    return {
        "location": {"lat": v["lat"], "lon": v["lon"], "elevation": v["elevation_m"],
                     "state": snap["state"], "district": snap["district"],
                     "block": snap["block"], "panchayat": v["name"]},
        ...
        "risks": [{"type": r.risk_type, "severity": _sev_label(r.severity),
                   ...
                   "description": r.description, "actions": r.recommended_actions} for r in risks],
        "advisory": gen.advisory_text, "weekly_plan": weekly, "sms_advisory": gen.sms_version,
    }
...
if use_demo:
    ...
    st.info("📦 Offline demo mode — advisory computed locally.")
...
location_parts = [x for x in [loc.get("panchayat"), loc.get("block"), loc.get("district"), loc.get("state")] if x]
...
with col_main:
    ...
    for day in advisory_data.get("weekly_plan", []):
        try:
            day_label = datetime.strptime(day.get("date", ""), "%Y-%m-%d").strftime("%a %d %b")
        except (ValueError, TypeError):
            day_label = day.get("date", "")
        priority = day.get("priority", "Low")
        icon = "🔴" if priority == "High" else "🟡" if priority == "Medium" else "🟢"
        with st.expander(f"{icon} {day_label} — {priority} Priority", expanded=(priority == "High")):
...
with col_risk:
    ...
    if risks:
        for risk in risks:
            ...
            icon = "🔴" if color == "red" else "🟠" if color == "orange" else "🟡" if color == "yellow" else "🟢"
            st.markdown(f"**{icon} {risk.get('type', 'Unknown')}** — {risk.get('severity', 'Low')}")
...
with col_sms2:
    ...
    st.caption("Via SMS gateway / WhatsApp API to registered farmers in their language.")
